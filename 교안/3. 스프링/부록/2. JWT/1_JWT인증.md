...
| 항목 | 내용 |
|---|---|
| 선수학습 | Day 16(REST API), Day 12(세션 로그인), 부록 3~4(시큐리티·필터 체인·`UserDetails` — 폼 로그인을 Security로 바꾼 상태를 가정) |
...
> 이 챕터는 17일 본 과정과 별도로 편성된 **부록**의 두 번째 챕터입니다(부록 1은 JPA). 본 과정은
> Day 12에서 `HttpSession`만으로 **세션 로그인**을 완성하고, 미니 프로젝트(Day 18)는 그
> 세션 로그인을 기본으로 요구합니다. 이 챕터는 **부록 3~4(Spring Security)를 먼저 마쳐
> 폼 로그인을 Security로 바꾼 상태**를 전제로, 그 위에 `/api/**` 구간만 토큰 기반으로 바꿉니다.
...
부록 3~4의 폼 로그인(또는 Day 12의 `HttpSession` 로그인)은 **서버가 세션(상태)을 저장**합니다.
...
```
(`UserDetailsService` + `PasswordEncoder` 는 부록 3 그대로 재사용.)
...
> 미니 프로젝트(Day 18)의 로그인은 세션(Day 12)이 기본이지만, 부록까지 마쳤다면 `/api/**`
> 구간만 JWT로 바꿔 두 방식을 한 프로젝트에서 비교해볼 수 있습니다.
