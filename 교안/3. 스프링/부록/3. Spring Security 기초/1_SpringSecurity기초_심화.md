# 부록 3. Spring Security 기초 — 심화 (경험자용)
...
커스텀 필터: `http.addFilterBefore(myFilter, UsernamePasswordAuthenticationFilter.class)` (부록 2. JWT).
...
```
`@PreAuthorize` 는 AOP(Day 14) — 내부 호출·public 규칙 동일. 컨트롤러 인가는 `authorizeHttpRequests`,
...
- 컨트롤러: `@AuthenticationPrincipal UserDetails user` (부록 4).
...
`@AuthenticationPrincipal MemberPrincipal me` 로 `me.getMember().getName()` 등 도메인 정보 접근(부록 4).
