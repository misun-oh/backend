# 부록 3. Spring Security 기초

| 항목 | 내용 |
|---|---|
| 선수학습 | Day 6(MVC·필터), Day 15(폼), Day 7(MyBatis), Day 12(세션 기반 로그인과 권한 — 이 챕터가 그걸 대체) |
...
> 이 챕터는 18일 본 과정과 별도로 편성된 **부록**의 세 번째 챕터입니다. 본 과정(Day 12)에서는
> `HttpSession`만으로 간단한 로그인을 직접 구현했습니다. 여기서는 그 자리를 Spring Security로
> 통째로 교체하면서, 권한별 접근 제어·CSRF 방어처럼 직접 구현 없이는 놓치기 쉬운 부분을 프레임워크에 맡깁니다.
> `MEMBER` 테이블은 Day 12와 같은 `MEMBER_ID`(로그인 아이디)를 그대로 쓰고, `ROLE` 컬럼 하나였던
> 권한만 다대다가 가능한 `MEMBER_ROLE` 테이블로 넓힙니다.
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .csrf(csrf -> csrf.ignoringRequestMatchers("/api/**"));   // API는 CSRF 제외(부록 2. JWT)
...
Day 12의 `MEMBER`(`MEMBER_ID`/`PASSWORD`/`MEMBER_NAME`/`ROLE`)에서 `ROLE` 컬럼만 떼어
다대다 테이블로 넓힙니다. `MEMBER_ID`는 그대로 두므로 기존 계정·비밀번호 해시는 손댈 필요가 없습니다.

```sql
ALTER TABLE MEMBER DROP COLUMN ROLE;   -- Day 12의 단일 ROLE 컬럼을 아래 테이블로 대체
ALTER TABLE MEMBER ADD COLUMN ENABLED CHAR(1) NOT NULL DEFAULT 'Y';
...
-- 기존 Day 12 데이터 이관 (지웠던 ROLE 값 기준으로 한 번만 실행)
INSERT INTO MEMBER_ROLE (MEMBER_ID, ROLE) VALUES ('admin', 'ADMIN'), ('user', 'USER');
```
...
public class MemberService {
    ...
    public String signup(SignupForm form) {
        if (memberMapper.existsById(form.getMemberId()))
...
시큐리티가 "이 아이디 사용자 정보 줘" 할 때 호출. 파라미터 이름은 시큐리티 인터페이스가
정한 `username`이지만, 넘어오는 값은 우리 `MEMBER_ID`(Day 12와 같은 로그인 아이디)입니다.
...
> 다음(부록 4): 로그인 사용자를 화면·로직에서 쓰고, 권한별로 메뉴를 보이거나 숨긴다 — 인증 심화.
