...
| 항목 | 내용 |
|---|---|
| 선수학습 | `Java 3.에노테이션`(리플렉션·애노테이션 처리), Day 4(빈·프록시), Day 3(로깅), Day 12(`@Transactional` 사용 경험) |
...
> **Day 12에서 `@Transactional` 애노테이션 하나만 붙였는데 커밋·롤백이 자동으로 됐던 것, 기억하시나요?**
> 그 마법이 오늘 배울 AOP입니다 — 메서드에 코드를 한 줄도 안 더했는데 어떻게 트랜잭션이 걸렸는지,
> 오늘 그 원리를 직접 만들어 보며 풉니다.
...
> 이 한계는 **Day 12 트랜잭션**에서 이미 마주쳤을 수도 있습니다: "한 서비스 메서드에서 같은 클래스의
> 다른 `@Transactional` 메서드를 호출해도 새 트랜잭션이 안 열린다" — 그 이유가 바로 이겁니다.

---

## 7. `@Transactional` 도 AOP다

우리가 Day 12에서 이미 써 본 `@Transactional`, 캐시의 `@Cacheable`, Spring Security의
`@PreAuthorize`(Day 17) 는 전부 **AOP 어드바이스**입니다. 프록시가 메서드 호출을 가로채:
