...
└── templates/
    ...
    ├── error.html             공통 에러 페이지 (Spring Boot 기본 위치)
    ├── error/                 403 · 404 · 500 상태코드별 에러 페이지
    ├── hr/                    사원/부서 CRUD · 대시보드 · 로그인 (스프링 과정 본 예제)
    ├── book/                  사원이 사내 도서를 대여 (캐러셀·별점·대여 흐름) · 로그인
    ├── shopping-mall/         사원이 자사 상품을 구매 (상품·장바구니·주문/결제)
    └── instagram/             회원이 일상을 공유하는 사내 SNS (피드·탐색·프로필)
```

- **검색/필터**는 `<form method="get">` + `name` 규약(`keyword`·`deptId`/`category`·`sort`·`page` …). 조건이 URL 에 남아 페이징·북마크·뒤로가기에 그대로 재사용됩니다.
- **처리 결과 메시지**는 `.alert`(component.css 15절), **결과 0건**은 `.empty-state`(16절). 서버는 PRG 리다이렉트로 플래시 메시지를 넘겨 `.alert` 에 렌더링합니다.

- HTML 은 `templates/`, CSS·JS 는 `static/css/` · `static/js/`. 링크는 **스프링식 절대경로**(`/css/basic.css`, `/js/component.js`).
- 각 앱 폴더(`templates/hr/` 등)에는 화면 HTML + `_menu.html`(공통 상단 메뉴 조각) + `README.md` 가 있습니다. 앱 전용 스타일은 `static/css/<앱>.css`.
