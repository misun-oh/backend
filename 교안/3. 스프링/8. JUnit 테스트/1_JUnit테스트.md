...
| 항목 | 내용 |
|---|---|
| 선수학습 | Day 7(EmpMapper, DB 연결), Java 기본(예외) |
...
- Day 7의 `EmpMapper` 가 **진짜 DB에 붙어** SELECT/INSERT 되는지 테스트로 확인할 수 있다.
...
Day 7에서 만든 `EmpMapper` 가 진짜로 동작하는지.
...
- 일반 코드에서 `@Transactional` = "정상 반환 시 커밋, 예외 시 롤백"(Day 12).
...
> 다음(Day 9): 이제 안심하고 `insert`/`update`/`delete` 와 조인 매핑을 만든다 — 테스트로 검증하며.
