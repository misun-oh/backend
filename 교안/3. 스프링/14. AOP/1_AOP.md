...
| 항목 | 내용 |
|---|---|
| 선수학습 | `Java 3.에노테이션`(리플렉션·애노테이션 처리), Day 4(빈·프록시), Day 3(로깅), Day 13(`@Transactional` 사용 경험) |
...
> **Day 13에서 `@Transactional` 애노테이션 하나만 붙였는데 커밋·롤백이 자동으로 됐던 것, 기억하시나요?**
...
> 이 한계는 **Day 13 트랜잭션**에서 이미 마주쳤을 수도 있습니다: "한 서비스 메서드에서 같은 클래스의
...
우리가 Day 13에서 이미 써 본 `@Transactional`, 캐시의 `@Cacheable`, Spring Security의
`@PreAuthorize`(부록 3) 는 전부 **AOP 어드바이스**입니다. 프록시가 메서드 호출을 가로채:
...
> 다음(Day 15): 잘못된 입력을 걸러내고 예외를 일관된 화면/응답으로 — 검증과 예외 처리.
