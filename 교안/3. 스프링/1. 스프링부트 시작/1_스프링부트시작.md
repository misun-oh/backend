...
| 이번 챕터 | 스프링 부트가 없애 준 것 → `start.spring.io` 프로젝트 생성 → 프로젝트 구조 → `@SpringBootApplication` → 내장 톰캣 실행 → `application.yml` (컨트롤러는 **[2_컨트롤러.md](2_컨트롤러.md)**, hr 프로토타입·DevTools는 **2. 프로토타입** 챕터) |
| 권장 진행 | 반나절 (2·3번 문서 포함 1일) |
...
| 결과물 | `main()` 실행 시 내장 톰캣이 떠서 `http://localhost:8080` 접속하면 **Whitelabel Error(404)** 가 뜨는 프로젝트 — 주소를 처리하는 컨트롤러는 [2_컨트롤러.md](2_컨트롤러.md)에서 이어서 만듭니다 |
...
> 컨트롤러 작성(`@Controller`/`@RestController`)·Thymeleaf 자동 구성의 학습목표는
> **[2_컨트롤러.md](2_컨트롤러.md)**, hr 프로토타입 붙이기·DevTools 는 **2. 프로토타입/1. 프로토타입.md**
> 에서 이어집니다.
...
> MyBatis·MySQL 드라이버는 DB를 붙이는 **Day 10**에서 추가합니다. 오늘은 웹 요청 처리까지만.
>
> **여기서 Lombok 체크를 놓쳤다면** 프로젝트를 다시 만들 필요 없습니다 — `build.gradle`에 두 줄만
> 추가하면 됩니다. **[3_의존성추가하기.md](3_의존성추가하기.md)** 참고.
...
dependencies {
    ...
    runtimeOnly 'com.mysql:mysql-connector-j'                                 // Day 7에서 실제 사용
...
- Initializr 가 만들어 준 것과 거의 같습니다. `mysql-connector-j` 만 미리 넣어 뒀습니다(Day 7 대비).
- **MyBatis**(`mybatis-spring-boot-starter:4.0.1`)·**검증**(`spring-boot-starter-validation`)·
  **BCrypt**(`spring-security-crypto`)는 각각 **Day 7 · 15 · 12** 에서 추가합니다(전체 Security
  프레임워크는 필요할 때 **부록 3**에서 선택 추가).
...
└── src
    ...
    │       ├── templates/                 Thymeleaf HTML (Day 10)
    │       └── application.properties     설정 파일 (→ yml로 바꿀 예정)
    └── test
        └── java/com/example/hr
            └── HrApplicationTests.java    기본 테스트 (Day 8)
```
...
아직 처리할 주소를 안 만들었을 뿐, 서버는 정상입니다. 주소는 **[2_컨트롤러.md](2_컨트롤러.md)** 에서 만듭니다.
...
- **스타터 빠뜨림** → `@RestController` 를 import 못 함, 톰캣이 안 뜸, 부팅 로그 직후 앱이 그냥 종료됨. `build.gradle` 에 `spring-boot-starter-webmvc` 가 있는지 확인. (왜 그런지 원리는 심화 1절 "웹 애플리케이션은 어떻게 '웹'이 되는가")
...
> 컨트롤러 관련 실수(뷰 이름이 그대로 응답됨 등)는 **[2_컨트롤러.md](2_컨트롤러.md)**,
...
> 다음: 컨트롤러를 만들어 방금 뜬 Whitelabel Error(404)를 실제 응답으로 바꾸는 **[2_컨트롤러.md](2_컨트롤러.md)**.
> 그 다음(Day 3)은 방금 `application.yml` 에 넣은 `logging.level` 을 제대로 다루는 **로깅 설정**.
