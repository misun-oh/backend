...
| 권장 진행 | 1일 |
| 결과물 | `/emps` 로 사원 목록 화면(간단한 HTML)이 뜨는 프로젝트. 본격 Thymeleaf 문법은 Day 10 |
...
뷰(`emp/detail.html`)에서 `${emp.empName}`, `${title}` 로 꺼내 씁니다(Day 10).
...
> 다음(Day 7): 이 `Model`에 담을 진짜 데이터를 DB에서 가져온다 — **MyBatis 시작과 DB 설정**.
> (`Model` 데이터를 실제 HTML로 그리는 **Thymeleaf**는 MyBatis CRUD까지 마친 뒤 **Day 10**에서 —
> 그때까지는 방금 만든 간단한 HTML로 확인합니다.)
