...
| 항목 | 내용 |
|---|---|
| 선수학습 | Day 6(MVC), Day 11(페이징), Day 15(검증·예외), HTTP 메서드·상태코드 |
...
| 인증 필요 | 401 | Day 12(로그인), 토큰 기반이면 부록 2(JWT) |
| 권한 없음 | 403 | Day 12(권한 체크), 부록 3(Security)로 대체 가능 |
...
> 화면용 `@ControllerAdvice`(Day 15)와 API용 `@RestControllerAdvice` 를 `basePackages` 로 나눠
> 둘이 안 겹치게 합니다.
...
> 다음(Day 17): 사진·문서 같은 **파일 업로드와 다운로드**.
