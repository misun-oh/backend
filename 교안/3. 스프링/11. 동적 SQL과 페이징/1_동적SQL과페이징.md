...
| 항목 | 내용 |
|---|---|
| 선수학습 | Day 9(EmpMapper·resultType), Day 10(Thymeleaf 화면), SQL `4. SUBQUERY`(LIMIT), Day 1(`EmpSearchCond`) |
...
- `#{kw}` 로 바인딩되므로 여전히 **파라미터 바인딩**(SQL 인젝션 안전)이고, DB 함수가 없어
  **어떤 DB로 바꿔도 그대로** 동작합니다. (같은 값을 여러 번 쓸 때 `CONCAT(...)` 을 반복 안 써도 되는 것도 장점.)
