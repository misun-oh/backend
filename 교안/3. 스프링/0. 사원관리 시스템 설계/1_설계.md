...
이 "누가 무엇을 할 수 있나"가 **권한(역할) 기반 접근 제어**(Day 12, 심화는 부록 3)의 밑그림입니다.
...
> 이번 그림에는 핵심 기능만 담고, 파일 저장소는 **Day 17(파일 업로드)** 에서 다시 그립니다.
...
- **DTO**(Data Transfer Object): 특정 화면·요청 **전용** 데이터 묶음.
  ...
  - `PageResult<T>` — 목록 + 전체 건수 + 페이지 정보 (Day 11)
...
- **검증은 Controller에서** 먼저 걸러 Service로 잘못된 값이 안 들어가게 한다(Day 15).
...
전체 건수(`COUNT`). 이 패턴은 SQL 과정 `4. SUBQUERY`의 페이징과 같고, Day 11 "동적 SQL과 페이징"에서 구현합니다.
...
├── config/                   설정 클래스 (WebConfig — 인터셉터 등록, MyBatisConfig, (SecurityConfig — 부록 3 적용 시) …)
...
- `mapper` 인터페이스는 `src/main/java`에, SQL을 적는 **XML은 `src/main/resources/mapper/`** 에 둡니다(Day 7).
...
|---|---|---|---|
| 유즈케이스 | 누가 무엇을 하려는가 (범위) | 액터 2, 유즈케이스 8 | 권한 설계(Day 12) |
| 화면 목록·흐름도 | 어떤 화면이 있고 어떻게 이동하는가 | 화면 6, 흐름 화살표 = 컨트롤러 메서드 | MVC·Thymeleaf(Day 6~10) |
| 클래스 다이어그램 | 클래스를 어떻게 나누고 연결하는가 | Controller→Service→Mapper + 도메인/DTO | 계층형 아키텍처(Day 5) |
| 시퀀스 다이어그램 | 요청 하나가 어떻게 흐르는가 | 등록·조회 흐름 | 검증(Day 15), 트랜잭션(Day 13), 페이징(Day 11) |
