...
-- 제약조건 - 테이블을 생성할때 추가, ALTER명령을 이용해서 추가
-- EMP_COPY 생성, 기본키 외래키 추가
CREATE TABLE EMP_COPY AS SELECT * FROM EMP;
-- DEPT테이블의 구조와 데이터를 모두 복사해서 DEPT_COPY테이블을 만들어 봅시다
CREATE TABLE DEPT_COPY AS SELECT * FROM DEPT;

-- 기본키 제약조건 추가
-- EMP_ID가 이미 중복된 데이터가 있는경우 오류발생 
...
ALTER TABLE EMP_COPY
	ADD CONSTRAINT FK_EMPCOPY_DEPT1 FOREIGN KEY (DEPT_ID) REFERENCES DEPT_COPY(DEPT_ID);
