...
- `학생 --> 학과`, `학생 --> 교수`는 **연관**입니다. `소속학과`/`지도교수`가 `학생`의 **필드**이기 때문이죠 — `@주입`이 붙어서 값이 나중에(생성자가 아니라 컨테이너가) 채워질 뿐, 필드로 오래 보유한다는 사실 자체는 그대로입니다. 지난 시간 `01_교안_GoF디자인패턴.md` 4-4절에서 다룬 "`@Autowired`도 이름과 달리 UML상으로는 연관"이라는 내용이 여기 그대로 적용됩니다.
